                ...
                const mealsDropdown = order.meals.map((meal) => `
                    <li>
                       ...
                       <button class="meal-done-btn" om-id="${meal.om_id}" 
                        ...
                        ${order.chef_id && order.order_status === 0 ? '' : 'disabled'}>
                        Done
                        </button>
                        ${meal.meal_id}
                    </li>
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            if (response2.ok) {
                                ...
                                // Change status text to 'Preparing'
                                const statusElement = orderRow.querySelector(".status span");
                                if (statusElement) {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            if (response2.ok) {
                                ...
                                // Disable all meal-done buttons for the order
                                document.querySelectorAll(`.meal-done-btn[data-order-id="${orderId}"]`)
                                    .forEach(btn => btn.disabled = true);
                            
                                // Disable the main done button
                                event.target.disabled = true;
                            }
                ...
                button.addEventListener("click", async (event) => {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            // Disable only the clicked meal button
                            thisButton.disabled = true;
                        
                            // Check if all meals are done for this order
