...
mealReportButton.addEventListener("click", function() {
    ...
    // Load unfiltered data by default
    fetch("/admin/reports/meals")
        .then((response) => response.json())
        .then((data) => {
            allMealsData = data;
            renderMealTable(allMealsData);
            mealReportModal.style.display = "block";
            this.classList.remove("loading");
        })
        .catch((error) => {
            console.error("Error fetching data:", error);
...
closeMealModal.addEventListener("click", function() {
    mealReportModal.style.display = "none";
});

window.addEventListener("click", function(event) {
    if (event.target === mealReportModal) {
        mealReportModal.style.display = "none";
    }
});

// Apply filters for meals report
...
// Add print functionality for meals report
document.addEventListener('DOMContentLoaded', function() {
    initMealPrintFunctionality();
});

function initMealPrintFunctionality() {
    const filterActions = document.querySelectorAll('.filter-actions')[1]; // Get the second filter actions div
    if (filterActions) {
        const printBtn = document.createElement('button');
        printBtn.id = 'printMealReportBtn';
        printBtn.className = 'print-btn';
        printBtn.innerHTML = '<img src="/Photo/icon/print.png" alt="Print"> Print Report';
        
        filterActions.appendChild(printBtn);
        printBtn.addEventListener('click', printMealReport);
    }
}
